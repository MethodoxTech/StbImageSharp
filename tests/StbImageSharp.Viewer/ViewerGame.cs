...
namespace StbImageSharp.Samples.MonoGame
{
    ...
    public class ViewerGame : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            using (FileStream stream = File.OpenRead(_filePath))
            {
                if (!_isAnimatedGif)
                {
                    ...
                    Texture2D texture = new(GraphicsDevice, image.Width, image.Height, false, SurfaceFormat.Color);
                ...
                {
                    _totalDelayInMs = 0;
                    foreach (AnimatedFrameResult image in ImageResult.AnimatedGifFramesFromStream(stream))
                    {
                        Texture2D texture = new(GraphicsDevice, image.Width, image.Height, false, SurfaceFormat.Color);
                        texture.SetData(image.Data);
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            {
                int passed = (int)(DateTime.Now - _started.Value).TotalMilliseconds;
