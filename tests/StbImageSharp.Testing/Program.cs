...
namespace StbImageSharp.Testing
{
    internal static class Program
    {
        ...
        {
            private readonly ConcurrentDictionary<string, int> _byExtension = new();
            private readonly ConcurrentDictionary<string, int> _byExtensionCount = new();
        ...
        private static LoadingTimes stbImageSharpTotal = new();
        private static LoadingTimes stbNativeTotal = new();
        private static LoadingTimes imageSharpTotal = new();
        ...
        private static LoadResult ParseTest(string name, LoadDelegate load)
        {
            ...
            int passed = EndWatch(sw) / LoadTries;
        ...
        {
            string[] files = Directory.EnumerateFiles(imagesPath, "*.*", SearchOption.AllDirectories).ToArray();
        ...
        private static void ThreadProc(string f)
        {
            ...
            try
            {
                ...
                LoadResult stbNativeResult = ParseTest(
                    ...
                    (out int x, out int y, out ColorComponents ccomp) =>
                    {
                        byte[] result = Native.load_from_memory(data, out x, out y, out int icomp,
                ...
                for (int i = 0; i < stbImageSharpResult.Data.Length; ++i)
